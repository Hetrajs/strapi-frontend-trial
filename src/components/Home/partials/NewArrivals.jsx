...
const ProductCard = React.memo(({ product, onProductClick }) => {
  ...
  const getCurrentImageSrc = useCallback(() => {
    ...
    const imageObj = product.colorVariants.find(v => v.color === currentColor)?.images[0];
    return imageObj ? imageObj.url : product.colorVariants[0].images[0]?.url || "";
  }, [hoveredColor, selectedColor, product.colorVariants]);
  ...
  const discount = calculateDiscount(product.colorVariants[0].price, product.colorVariants[0].originalPrice);
      ...
      <div className="bg-neutral-100 rounded-2xl overflow-hidden shadow-sm h-full">
        <div onClick={() => onProductClick(product)} style={{ cursor: 'pointer' }}>
          <div className="relative">
            <img src={getCurrentImageSrc()} alt={product.name} className="w-full h-96 sm:h-56 md:h-96 object-cover" />
            {discount > 0 && (
          ...
          <div className="p-4">
            ...
            <h3 className="Asul text-[14px] leading-[18px] text-sm font-medium mb-1">{product.name}</h3>
            <div className="Fira flex items-center">
              <span className="text-lg font-bold mr-2">{formatIndianPrice(product.colorVariants[0].price)}</span>
