  ...
  useEffect(() => {
    if (showExitWarning) {
      const userChoice = window.confirm(
        ...
        //  "Warning: Reloading will reset your order. Continue?"
      );
      if (userChoice) {
    ...
    <div className="max-w-md md:max-w-[550px] mx-auto bg-white p-6 rounded-lg shadow-md">
      <form className="space-y-4" onSubmit={handleSubmit}>
        ...
        <button 
            ...
            className={`w-fit bg-black text-white py-2 px-3 rounded-md transition-opacity duration-300 ${
              !isFormComplete() || isFormSubmitted ? 'opacity-50 cursor-not-allowed' : 'hover:opacity-80'
            }`}
          >
            Confirm details
          </button>
        <h1 className='text-[12px] flex items-center gap-2 text-red-400'><Info className='cursor-pointer' />Don't Refresh page after confirming details. </h1>
        </div>
      </form>
      <ToastContainer
        position="top-right"
